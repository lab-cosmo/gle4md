...
int main(int argc, char **argv)
{
    ...
    {           
        std::valarray<double> w(np), kw(np), hw(np), tq2(np), tp2(np), th(np), q2(np), p2(np), pq(np), hdist(np), lfp(np),
                 ...
                 sqq(np), spp(np), rp_PWw0q(np), rp_PWgq(np), rp_PWshapeq(np), rp_PWw0p(np), rp_PWgp(np), rp_PWshapep(np) ;
        ...
        for (unsigned long ip=0; ip<np; ip++)
        {
            ...
            if (wrpmd > 0) make_rpmodel_abc(iA, iBBT, w[ip],  wrpmd, rpalpha, abcrp);
            ...
            if (wrpmd>0)  harm_shape(abcrp, rp_PWw0q[ip], rp_PWgq[ip], rp_PWshapeq[ip], 0);
            if (wrpmd>0)  harm_shape(abcrp, rp_PWw0p[ip], rp_PWgp[ip], rp_PWshapep[ip], 1);
        }
        if (!ftex)
        {
